...
days = unique(haplotypes$day)

for (d in days){
  haplotypes %>% filter(sample == samples_compared[i] & day == d) -> sample_data
  ...
  for (pb_hap in unique(sample_data$pb_haplotype)){
    print(unique(sample_data$pb_haplotype))
    hap_num = hap_num + 1
    ...
    file_name = paste("~/Projects/flu_project/haplotype_figures/", "Sample ", samples_compared[i], "_", d, "_", "haplo", hap_num, "_illumina_linkages.pdf", sep="")
